...
public class GrapplingHookPhysicsContext {
	...
	public GrapplingHookPhysicsContext(int grapplehookEntityId, int entityId, Level world, int controllerId, GrappleCustomization custom) {
		...
		if (grapplehookEntityId != -1) {
			...
			if (grapplehookEntity != null && grapplehookEntity.isAlive() && grapplehookEntity instanceof GrapplinghookEntity) {
				this.addHookEntity((GrapplinghookEntity)grapplehookEntity);
			} else {
		...
		if (this.attached) {
			if(entity != null) {
				...
				if (this.custom.motor) {
					if (GrappleModClient.get().isKeyDown(GrappleModKey.key_motoronoff) && this.custom.motorwhencrouching) {
						motor = true;
					} else if (!GrappleModClient.get().isKeyDown(GrappleModKey.key_motoronoff) && this.custom.motorwhennotcrouching) {
						motor = true;
					}
				}
							...
							if (anchor.y > playerpos.y) {
								...
								if (climbup != 0) {
										if (dist + distToAnchor < maxLen || climbup > 0 || maxLen == 0) {
											...
											Vec additionalmovementdown = spherevec.changeLen(-climbup * GrappleModConfig.getConf().grapplinghook.other.climb_speed).proj(new Vec(0,1,0));
...
//									vec pullxy = new vec(pull.x, 0, pull.z);
	...
	public void calcTaut(double dist, GrapplinghookEntity hookEntity) {
		if (hookEntity != null) {
	...
	public double getJumpPower(Entity player, Vec spherevec, GrapplinghookEntity hookEntity) {
		...
		double current_speed = GrappleModConfig.getConf().grapplinghook.other.rope_jump_at_angle ? -motion.distAlong(spherevec) : motion.y;
		if (current_speed > 0) {
...
//    	Block b = blockstate.getBlock();
	...
	public boolean isWallRunning() {
		...
		if (current_speed < GrappleModConfig.getConf().enchantments.wallrun.wallrun_min_speed) {
			isOnWall = false;
			return false;
		}
	...
	public boolean applyWallrun() {
		...
		if (wallrun && !GrappleModClient.get().isKeyDown(GrappleModKey.key_jumpanddetach)) {
			...
			if (ticksSinceLastWallrunSoundEffect > GrappleModConfig.getClientConf().sounds.wallrun_sound_effect_time_s * 20 * GrappleModConfig.getConf().enchantments.wallrun.wallrun_max_speed / (vel + 0.00000001)) {
		...
		// jump
		boolean isjumping = GrappleModClient.get().isKeyDown(GrappleModKey.key_jumpanddetach) && isOnWall;
		isjumping = isjumping && !playerJump; // only jump once when key is first pressed
		playerJump = GrappleModClient.get().isKeyDown(GrappleModKey.key_jumpanddetach) && isOnWall;
		if (isjumping && wallrun) {
			...
			if (wallDirection != null) {
				jump.add_ip(wallDirection.mult(-GrappleModConfig.getConf().enchantments.wallrun.wall_jump_side));
			}
			motion.add_ip(jump);
